function mostrar5(){
    document.getElementById('cronograma').style.display = "block";
    document.getElementById('estudiantes').style.display = "none";
    document.getElementById('prestamo').style.display = "none";
    document.getElementById('herramientas').style.display = "none";
    document.getElementById('actividades').style.display = "none";
}

function mostrar6(){
    document.getElementById('estudiantes').style.display = "block";
    document.getElementById('cronograma').style.display = "none";
    document.getElementById('prestamo').style.display = "none";
    document.getElementById('herramientas').style.display = "none";
    document.getElementById('actividades').style.display = "none";
}

function mostrar7(){
    document.getElementById('prestamo').style.display = "block";
    document.getElementById('estudiantes').style.display = "none";
    document.getElementById('cronograma').style.display = "none";
    document.getElementById('herramientas').style.display = "none";
    document.getElementById('actividades').style.display = "none";
}

function mostrar8(){
    document.getElementById('herramientas').style.display = "block";
    document.getElementById('estudiantes').style.display = "none";
    document.getElementById('prestamo').style.display = "none";
    document.getElementById('cronograma').style.display = "none";
    document.getElementById('actividades').style.display = "none";
}

function mostrar9(){
    document.getElementById('actividades').style.display = "block";
    document.getElementById('estudiantes').style.display = "none";
    document.getElementById('prestamo').style.display = "none";
    document.getElementById('herramientas').style.display = "none";
    document.getElementById('cronograma').style.display = "none";
}

function mostrar10(){
    document.getElementById('listado_act').style.display = "block";
    document.getElementById('listado_est').style.display = "none";
}

function mostrar11(){
    document.getElementById('listado_est').style.display = "block";
    document.getElementById('listado_act').style.display = "none";
}

function mostrar12(){
    document.getElementById('listado_act').style.display = "block";
    document.getElementById('cronograma').style.display = "none";
    document.getElementById('prestamo').style.display = "none";
    document.getElementById('herramientas').style.display = "none";
    document.getElementById('actividades').style.display = "none";
    document.getElementById('listado_est').style.display = "none";
}